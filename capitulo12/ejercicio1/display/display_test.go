...
func TestFormatAsMapKey(t *testing.T) {
	var tests = []struct {
		in   interface{}
		want string
	}{
		...
		{struct{ x int }{42}, "{42}"},
		{struct{ x, y int }{}, "{0 0}"},
		{struct{ x, y int }{0, 0}, "{0 0}"},
		{struct{ x, y int }{42, 420}, "{42 420}"},
		{struct{ x string }{}, "{\"\"}"},
		{struct{ x string }{""}, "{\"\"}"},
		{struct{ x string }{"foo"}, "{\"foo\"}"},
		{struct{ x, y string }{}, "{\"\" \"\"}"},
		{struct{ x, y string }{"", ""}, "{\"\" \"\"}"},
		{struct{ x, y string }{"foooo", "🍣"}, "{\"foooo\" \"🍣\"}"},
		...
		{[1]string{}, "[\"\"]"},
		{[3]string{}, "[\"\" \"\" \"\"]"},
		{[3]int{42, 420, 4200}, "[42 420 4200]"},
		{[1]string{}, "[\"\"]"},
		{[3]string{"f", "foo", "にほんご 🍣"}, "[\"f\" \"foo\" \"にほんご 🍣\"]"},
	}

	for _, test := range tests {
		if got := formatAsMapKey(reflect.ValueOf(test.in)); got != test.want {
			t.Errorf("formatAsMapKey(%v) = %q, want %q", test.in, got, test.want)
		}
	}
}
